...
def sanitize_history(messages: list) -> list:
    ...
    ⚠️ 副作用：
        通过 `messages[:] = cleaned` 原地替换列表内容，
        原列表对象本身不变，但内容会被替换。
        返回的也是同一个 messages 对象（便于链式调用）。

    ⚠️ 调用方警告：
        如果你持有 messages 的引用，调用此函数后该引用看到的内容也会变。
        如果你需要保留原始状态，请先 `copy.deepcopy(messages)`。

    ⚠️ 幂等性：
        对同一 messages 重复调用是安全的——第二次调用不会做任何修改。
    """
    called_ids: set = set()
    for m in messages:
